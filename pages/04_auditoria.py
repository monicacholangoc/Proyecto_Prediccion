...
# CSS para tarjetas compactas y botones legibles
st.markdown("""
<style>
...
.feat-lbl {
    ...
    color:#7a8499; text-transform:uppercase;
    letter-spacing:0.04em; margin-bottom:0.05rem;
}
.feat-val {
    font-size:0.88rem; font-weight:700;
    color:#1a2236; line-height:1.2; margin-bottom:0.05rem;
}
...
.feat-badge-green { background:#15803d; }
.feat-badge-amber  { background:#b45309; }
.feat-badge-blue   { background:#1d4ed8; }
.feat-badge-gray   { background:#64748b; }
...
with tab1:
    ...
    with left_col:
        ...
        validate_context = st.toggle(
            ...
            help=(
                "ON: detecta si la resena no menciona ningun termino alimenticio. "
                "Sin contexto de alimento la probabilidad cae a 0.05.\n"
        ...
        review_text = st.text_area("Texto de la resena", height=200,
        ...
        if analyze_clicked:
            if not is_non_empty_text(review_text):
                ...
                st.session_state["_toggle_prev_val"]  = validate_context  # guardar estado actual
                if "error" in api_r:
        ...
        # Recalcular en vivo si el toggle cambio respecto al analisis anterior
    ...
    with right_col:
        ...
        # Longitud
        lb = "feat-badge-green" if rl > 60 else "feat-badge-amber"
        ll = "Adecuada" if rl > 60 else "Corta"
        # Sentimiento
        if sent is not None:
            sl = "Positivo" if sent > 0.05 else ("Negativo" if sent < -0.05 else "Neutro")
            sb = "feat-badge-green" if sent > 0.05 else ("feat-badge-amber" if sent < -0.05 else "feat-badge-blue")
            ss = f"{sent:.2f}"
        else:
            sl, sb, ss = "—", "feat-badge-gray", "—"
        # Estrellas
        filled = "&#9733;" * int(sv); empty = "&#9734;" * max(0, 5-int(sv))
        # Coherencia
        cl = "Coherente" if not incoh else "Incoherente"
        cb = "feat-badge-green" if not incoh else "feat-badge-amber"
        ...
        <div class="feat-grid">
            <div class="feat-card">
                ...
                <div class="feat-cap">Umbral: &ge;80</div>
            ...
            <div class="feat-card">
                ...
                <span class="feat-badge feat-badge-blue">{sv}/5</span>
            </div>
            <div class="feat-card">
        ...
        if lr:
            ...
            if lr.get("context_validation_enabled"):
                ...
                st.markdown(
                    ...
                    f'<b>Detectadas:</b> {ctx} | <b>Ajenas:</b> {tech}</div></div>',
                    unsafe_allow_html=True,
                )
        else:
...
with tab2:
    ...
    st.markdown(
        ...
        '<div class="highlight-body">Columnas necesarias: '
        '<code>ProductId</code>, <code>ProfileName</code>, <code>Score</code>, <code>Text</code>.<br>'
        'El sistema calcula utilidad, sentimiento e incoherencia por fila.</div></div>',
        unsafe_allow_html=True,
    )
    ...
    if batch_result is not None and not batch_result.empty:
        ...
        with m1: render_metric_card("Total", str(len(batch_result)), "Resenas en el lote")
